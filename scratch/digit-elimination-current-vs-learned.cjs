...
function main() {
  ...
  for (const market of MARKETS) {
    ...
    for (const side of sides) {
      ...
      for (const fold of folds) {
        const trainSamples = buildSamples({ rowsByMarket, rows, market, side, startIndex: 0, endIndex: fold.validationStart });
        ...
        foldRows.push({
          ...
          currentEdgeVsRandom: current.accuracy - random,
        });
      }
      ...
      const randomAgg = foldRows.reduce((sum, fold) => sum + fold.random * fold.learned.n, 0) /
      ...
      markets.push({
        ...
        learnedLosesCurrent: foldRows.filter((fold) => fold.learnedEdgeVsCurrent < 0).length,
      });
    }
  }
  ...
  for (const side of sides) {
    ...
    const learned = rows.reduce((bucket, row) => {
      bucket.n += row.learned.n;
      bucket.correct += row.learned.correct;
      bucket.wrong += row.learned.wrong;
      bucket.perfect += row.learned.perfect;
      return bucket;
    }, { n: 0, correct: 0, wrong: 0, perfect: 0 });
    const current = rows.reduce((bucket, row) => {
      bucket.n += row.current.n;
      bucket.correct += row.current.correct;
      bucket.wrong += row.current.wrong;
      bucket.perfect += row.current.perfect;
      return bucket;
    }, { n: 0, correct: 0, wrong: 0, perfect: 0 });
    const random = rows.reduce((sum, row) => sum + row.random * row.learned.n, 0) /
      Math.max(1, rows.reduce((sum, row) => sum + row.learned.n, 0));
    totals[side] = {
      learned: finalize(learned),
      current: finalize(current),
      random,
      learnedWinsCurrent: rows.reduce((sum, row) => sum + row.learnedWinsCurrent, 0),
      learnedLosesCurrent: rows.reduce((sum, row) => sum + row.learnedLosesCurrent, 0),
    };
  }

  const output = { generatedAt: new Date().toISOString(), foldsPerMarketSide: 4, totals, markets };
  const outPath = path.join(process.cwd(), "scratch", "digit-elimination-current-vs-learned-output.json");
  fs.writeFileSync(outPath, JSON.stringify(output, null, 2));

  const pct = (value) => `${(value * 100).toFixed(1)}%`;
  console.log("# Current Predictor vs Learned Digit Elimination");
  console.log(`Output: ${outPath}`);
  for (const side of sides) {
    const row = totals[side];
    console.log(`${side}: random ${pct(row.random)}, current ${pct(row.current.accuracy)}, learned ${pct(row.learned.accuracy)}, learned fold W/L ${row.learnedWinsCurrent}/${row.learnedLosesCurrent}`);
  }
  console.log("\nLearned strongest vs current:");
  markets
    .sort((a, b) => (b.learned.accuracy - b.current.accuracy) - (a.learned.accuracy - a.current.accuracy))
    .slice(0, 12)
    .forEach((row) => {
      console.log(`${row.market} ${row.side}: current ${pct(row.current.accuracy)}, learned ${pct(row.learned.accuracy)}, random ${pct(row.random)}, delta ${pct(row.learned.accuracy - row.current.accuracy)}`);
    });
}
